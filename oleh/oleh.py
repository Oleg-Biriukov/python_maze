...
class MazeGenerator(BaseModel):
    ...
    def render_maze(self):
        def get_corner(y, x):
            ...
            if y > 0:
                if x < self.width:
                    up = bool(self._maze[y - 1][x].walls.value & 0b1000) #left 1000
                else:
                    up = bool(self._maze[y - 1][x - 1].walls.value & 0b0010)#right 0010

            if y < self.height:
                if x < self.width:
                    down = bool(self._maze[y][x].walls.value & 0b1000) # left 1000
                else:
                    down = bool(self._maze[y][x - 1].walls.value & 0b0010) # right 1000

            if x > 0:
                if y < self.height:
                    left = bool(self._maze[y][x - 1].walls.value & 0b0001) # top 0001
                else:
                    left = bool(self._maze[y - 1][x - 1].walls.value & 0b0100) # bottom 0100

            if x < self.width:
                if y < self.height:
                    right = bool(self._maze[y][x].walls.value & 0b0001) # top 0001
                else:
                    right = bool(self._maze[y - 1][x].walls.value & 0b0100) # bottom 0100
